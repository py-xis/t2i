...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.cache_k = None
        self.cache_v = None
        self.curr_cache_k = None
        self.curr_cache_v = None
    ...
    def forward(
        ...
        patch_attn: Optional[bool] = False,
        run_with_cache: Optional[bool] = False,
        timestep: Optional[torch.LongTensor] = None,
        attn_heads_idx_to_patch: Optional[List[int]] = None,
        batch_num: Optional[int] = None,
        is_last_timestep: Optional[bool] = False,
        timestep_start_patching: Optional[int] = 0,
        encoder_hidden_states_B: Optional[torch.Tensor] = None,
        **cross_attention_kwargs,
        ...
        return self.processor(
            ...
            encoder_hidden_states_B=encoder_hidden_states_B,
            **cross_attention_kwargs,
        )
...
class AttnAddedKVProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if run_with_cache and patch_attn:
            encoder_hidden_states_key_proj = attn.add_k_proj(encoder_hidden_states)
            encoder_hidden_states_value_proj = attn.add_v_proj(encoder_hidden_states)
            ...
            # save acts to cache
            if attn.curr_cache_k is None and attn.curr_cache_v is None:
                # add timestep dim
                attn.curr_cache_k = encoder_hidden_states_key_proj.cpu().unsqueeze(0)
                attn.curr_cache_v = encoder_hidden_states_value_proj.cpu().unsqueeze(0)
            else:
                attn.curr_cache_k = torch.cat(
                    [
                        attn.curr_cache_k,
                        encoder_hidden_states_key_proj.cpu().unsqueeze(0),
                    ],
                    dim=0,
                )
                attn.curr_cache_v = torch.cat(
                    [
                        attn.curr_cache_v,
                        encoder_hidden_states_value_proj.cpu().unsqueeze(0),
                    ],
                    dim=0,
                )
                if is_last_timestep:
                    # we can merge the cache
                    if attn.cache_v is None and attn.cache_k is None:
                        attn.cache_k = attn.curr_cache_k
                        attn.cache_v = attn.curr_cache_v
                    else:
                        attn.cache_k = torch.cat([attn.cache_k, attn.curr_cache_k], dim=1)
                        attn.cache_v = torch.cat([attn.cache_v, attn.curr_cache_v], dim=1)
                    attn.curr_cache_k = None
            ...
            if attn.cache_k is None or attn.cache_v is None:
                ...
                encoder_hidden_states_key_proj[
                    ...
                    :,
                    :,
                ].to(encoder_hidden_states_key_proj.device)
                encoder_hidden_states_value_proj[
                    ...
                    :,
                    :,
                ].to(encoder_hidden_states_value_proj.device)
            else:
                # patch all heads
...
class JointAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        key = attn.to_k(hidden_states)
        value = attn.to_v(hidden_states)
        # reshape for single head patching
        # logic: store activations in curr_cache until all timesteps, then merge with cache
        head_dim = encoder_hidden_states.shape[-1] // attn.heads
        ...
        if run_with_cache and patch_attn:
            encoder_hidden_states_key_proj = attn.add_k_proj(encoder_hidden_states)
            encoder_hidden_states_value_proj = attn.add_v_proj(encoder_hidden_states)
            # reshape for patching single heads
            ...
            encoder_hidden_states_value_proj = encoder_hidden_states_value_proj.transpose(1, 2).reshape(
                batch_size, -1, attn.heads * head_dim
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            ...
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
            if encoder_hidden_states_B is not None:
                encoder_hidden_states_B = attn.norm_encoder_hidden_states(encoder_hidden_states_B)

        inner_dim = attn.inner_kv_dim
        head_dim = inner_dim // attn.heads
        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

        if run_with_cache and patch_attn:
            # caching
            # compute and save acts to cache
            key = attn.to_k(encoder_hidden_states)
            value = attn.to_v(encoder_hidden_states)
            key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            ...
            if attn.curr_cache_k is None and attn.curr_cache_v is None:
                # add timestep dim
                attn.curr_cache_k = key.cpu().unsqueeze(0)
                attn.curr_cache_v = value.cpu().unsqueeze(0)
            else:
                attn.curr_cache_k = torch.cat(
                    [attn.curr_cache_k, key.cpu().unsqueeze(0)],
                    dim=0,
            ...
            if encoder_hidden_states_B is not None:
                key = attn.to_k(encoder_hidden_states_B)
                value = attn.to_v(encoder_hidden_states_B)
                key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
                value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            else:
                # patching
                if attn.cache_k is None or attn.cache_v is None:
                    ...
                    # patch all heads
                    key = attn.cache_k[timestep, batch_num * batch_size : (batch_num + 1) * batch_size].to(
                        query.device
                    )
                    value = attn.cache_v[timestep, batch_num * batch_size : (batch_num + 1) * batch_size].to(
                        query.device
                    )
        else:
            # default
