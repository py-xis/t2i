...
from src.pipeline_safe_stable_diffusion_xl import SafeStableDiffusionXLPipeline
...
SAVE_DIR = (
    f"results_sdxl_safe/toxic/edit/"
    f"{START_TIME}_"
    f"seed_{SEED}_"
...
pipe = SafeStableDiffusionXLPipeline.from_pretrained(
...
np.save(os.path.join(SAVE_DIR, "Safe.npy"), original_images_B)
...
original_images_B_df = pd.DataFrame(
    ...
    index=["Safe" for _ in range(len(prompts_A))],
)
original_images_B_df["Block_patched"] = ["Safe" for _ in range(len(prompts_B))]
