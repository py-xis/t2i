"""Timestep search with DeepFloyd-IF model on SimpleBench TOP100."""
...
from src.prepare_glyph import prepare_prompts_glyph_simple_bench_top_100
...
SAVE_DIR = (
    f"results_if/glyph_simple/timestep_search/"
    f"{START_TIME}_"
    f"seed_{SEED}_"
    ...
    f"max_timestep_{MAX_TIMESTEP}_"
...
for timestep_start in tqdm(range(MAX_TIMESTEP)):
    ...
    patched_images_df["Block_patched"] = [
        f"A{set_to_string(ATTENTIONS_TO_PATCH)}_t_{timestep_start}"
    ...
    np.save(
        os.path.join(
            SAVE_DIR,
            f"A{set_to_string(ATTENTIONS_TO_PATCH)}_t_{timestep_start}.npy",
        ),
        patched_images,
    )
