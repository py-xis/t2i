...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    Returns:
        `Tuple[torch.TDiffusionPipelineensor, int]`: A tuple where the first element is the timestep schedule from the scheduler and the
...
class SafeStableDiffusion3Pipeline(
    ...
    def __call__(
        ...
        if enable_safety_guidance:
            (
                ...
                safety_negative_prompt_embeds,
                safety_pooled_prompt_embeds,
                safety_negative_pooled_prompt_embeds,
            ) = self.encode_prompt(
                prompt=self.safety_concept,
                prompt_2=None,
                prompt_3=None,
                do_classifier_free_guidance=self.do_classifier_free_guidance,
                device=device,
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    if enable_safety_guidance:
                        ...
                        noise_pred = (
                            noise_pred_uncond + self.guidance_scale * noise_guidance
                        )
                    else:
