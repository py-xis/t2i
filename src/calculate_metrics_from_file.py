...
def calculate_metrics(
    ...
    ssim_val_no_text = ssim(
        torch.from_numpy(original_images_A_no_text).permute((0, 3, 1, 2)),
        torch.from_numpy(images_no_text).permute((0, 3, 1, 2)),
        data_range=255,
        size_average=False,
    ).numpy()
...
if __name__ == "__main__":
    ...
    prompts_A, prompts_B = prepare_prompts_glyph_simple_bench(n_samples_per_prompt=1)
    ...
    patched_images_df["Block_patched"] = ["P2P" for _ in range(len(prompts_A))]
