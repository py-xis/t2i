...
SAVE_DIR = f"results_sdxl/wiki398_TH_{START_TIME}"
ATTENTIONS_TO_PATCH = [
    55,
    56,
...
HEADS_TO_PATCH = {
    55: [2, 3, 4, 5, 7, 8, 10, 12, 14, 16, 17, 18, 19],
    56: [1, 2, 3, 6, 10, 11, 13, 15, 16, 17, 18, 19],
}
...
prompts_A, prompts_B = prepare_prompts_wiki398("data/wiki398.txt")
...
patched_images = sample(
    ...
    run_with_cache=False,
    attn_idx_to_patch=ATTENTIONS_TO_PATCH,
    attn_heads_idx_to_patch=HEADS_TO_PATCH,
)
...
patched_images_df["Block_patched"] = [
    f"T{ATTENTIONS_TO_PATCH[0]}H{set_to_string(HEADS_TO_PATCH[ATTENTIONS_TO_PATCH[0]])}+T{ATTENTIONS_TO_PATCH[1]}H{set_to_string(HEADS_TO_PATCH[ATTENTIONS_TO_PATCH[1]])}"
...
np.save(
    os.path.join(
        SAVE_DIR,
        f"T{ATTENTIONS_TO_PATCH[0]}H{set_to_string(HEADS_TO_PATCH[ATTENTIONS_TO_PATCH[0]])}+T{ATTENTIONS_TO_PATCH[1]}H{set_to_string(HEADS_TO_PATCH[ATTENTIONS_TO_PATCH[1]])}.npy",
    ),
