...
if __name__ == "__main__":
    SAVE_DIR = (
        f"results_if/{BENCHMARK}/edit/diff_templ_{USE_DIFFERENT_TEMPLATES}/"
        f"{START_TIME}_"
        f"seed_{SEED}_"
    ...
    if accelerator.is_main_process:
        np.save(os.path.join(SAVE_DIR, "Model.npy"), all_patched_images)

    if accelerator.is_main_process:
        patched_images_stats = calculate_metrics(
            all_original_images_A,
            original_images_A_feats,
            all_patched_images,
            [p["text"] for p in prompts_A],
