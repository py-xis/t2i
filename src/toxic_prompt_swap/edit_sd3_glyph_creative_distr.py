...
if __name__ == "__main__":
    SAVE_DIR = (
        f"results_sd3/glyph_toxic/prompt_swap/"
        f"{START_TIME}_"
        f"seed_{SEED}_"
    ...
    with accelerator.split_between_processes(prompts_indices) as device_indices:
        ...
        all_B_images.extend(B_images)
    accelerator.wait_for_everyone()
    all_B_images = gather_object(all_B_images)
    all_B_images = np.array(all_B_images)

    if accelerator.is_main_process:
        logging.info("Calculating metrics ...")
        np.save(os.path.join(SAVE_DIR, "B.npy"), all_B_images)
        B_images_metrics = calculate_metrics(
            all_original_images_A,
            original_images_A_feats,
            all_B_images,
            [p["text"] for p in prompts_A],
        ...
        B_images_df = pd.DataFrame(
            B_images_metrics,
        )
        B_images_df["Block_patched"] = ["B" for _ in range(len(prompts_A))]
        all_metrics_df = pd.concat([all_metrics_df, B_images_df])
        all_metrics_df.to_csv(os.path.join(SAVE_DIR, "metrics.csv"))
        logging.info("Finito!")
