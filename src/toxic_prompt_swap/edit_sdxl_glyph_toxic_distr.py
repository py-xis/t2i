...
SAVE_DIR = (
    f"results_sdxl/{BENCHMARK}/prompt_swap/"
    f"{START_TIME}_"
    f"seed_{SEED}_"
...
if __name__ == "__main__":
    ...
    with accelerator.split_between_processes(prompts_indices) as device_indices:
        ...
        all_samples_B.extend(samples_B)
    accelerator.wait_for_everyone()

    all_samples_B = gather_object(all_samples_B)
    all_samples_B = np.array(all_samples_B)
    print(all_samples_B.shape)

    if accelerator.is_main_process:
        logging.info("Calculating metrics ...")
        samples_B_metrics = calculate_metrics(
            all_original_images_A,
            original_images_A_feats,
            all_samples_B,
            [p["text"] for p in prompts_A],
        ...
        samples_B_df = pd.DataFrame(
            samples_B_metrics,
        )
        samples_B_df["Block_patched"] = [f"p_B" for _ in range(len(prompts_A))]

        all_metrics_df = pd.concat([all_metrics_df, samples_B_df])
