...
if __name__ == "__main__":
    SAVE_DIR = (
        f"results_if/{BENCHMARK}/prompt_swap/"
        f"{START_TIME}_"
        f"seed_{SEED}_"
    ...
    if accelerator.is_main_process:
        ...
        original_images_A_feats = extract_all_images(
            all_original_images_A, clip_model, accelerator.device, batch_size=BATCH_SIZE
        )
    ...
    with accelerator.split_between_processes(prompts_indices) as device_indices:
        ...
        all_prompt_B_images.extend(prompt_B_images)
    accelerator.wait_for_everyone()
    all_prompt_B_images = gather_object(all_prompt_B_images)

    all_prompt_B_images = np.array(all_prompt_B_images)

    if accelerator.is_main_process:
        np.save(os.path.join(SAVE_DIR, "p_B.npy"), all_prompt_B_images)

    if accelerator.is_main_process:
        prompt_B_images_stats = calculate_metrics(
            all_original_images_A,
            original_images_A_feats,
            all_prompt_B_images,
            [p["text"] for p in prompts_A],
        ...
        prompt_B_images_df = pd.DataFrame(
            prompt_B_images_stats,
        )
        prompt_B_images_df["Block_patched"] = [f"OURS" for _ in range(len(prompts_A))]

        all_metrics_df = pd.concat([all_metrics_df, prompt_B_images_df])
