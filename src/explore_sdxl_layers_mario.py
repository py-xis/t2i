...
def get_noise_per_prompt(generator, device):
    return torch.randn(
...
def get_ocrs(pil_images):
    ocr_model = get_ocr_easyocr(use_cuda=True)
    ocr_outs = []
    for img in pil_images:
        ret = get_text_easyocr(ocr_model, img)
        ocr_outs.append(ret)
    return ocr_outs
...
def load_model(sdxl_path, device="cpu"):
...
def sample_batched(
    ...
    for idx_start in loop:
        ...
        images = pipeline(
            ...
            generator=generator,
            latents=noises[idx_start:idx_end].to(pipeline.device),
...
def write_experiments_results(layers_stats, dir_path):
    with open(f"{dir_path}/metrics.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(
            [
                "Model Name",
...
def sample_trace(
    ...
    with distributed_state.split_between_processes(prompt_indices) as rank_prompts:
        all_noises = torch.cat(
            [
                get_noise_per_prompt(generator=generator, device=device)
                for _ in range(len(rank_prompts))
            ]
        )
        all_a_prompts = [
    ...
    if distributed_state.is_local_main_process:
        logging.info("Rank 0 finished, waiting for other ranks...")
    distributed_state.wait_for_everyone()
    all_images = gather_object(all_images)
    logging.info(f"Sampled {len(all_images)} images")
    return all_images


def run_experiment(experiment_name, prompt_file, output_dir):
    # loading
    tracing_layers, tracing_names = get_experiment(experiment=experiment_name)
    logging.info(f"Experiment: {experiment_name}")

    # models/data
    a_prompts, b_prompts = prepare_prompts_mario_bechmark(prompt_file)
    logging.info(f"Prompts: {prompt_file}")
    logging.info(f"Len of prompts: {len(a_prompts)}")
    ...
    for idx, b_layers in enumerate(tracing_layers):
        ...
        name = bl_id.upper()
        layers_path = f"{output_dir}/{name}"
        os.makedirs(layers_path, exist_ok=True)
        if distributed_state.is_main_process:
            logging.info(f">>>> Injection: {experiment_name} -> {name}")
    ...
    if distributed_state.is_main_process:
        write_experiments_results(layers_stats, dir_path=output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["blocks", "transformers", "layers", "2layers", "causal", "default"],
    )
    parser.add_argument("--prompt_file", type=str, help="Prompt file", required=True)
