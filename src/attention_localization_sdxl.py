"""Localization of cross-attention layers in SD-XL model responsible for text generation."""
    ...
    with tqdm(total=len(prompts)) as pbar:
        for batch_num, batch_start in enumerate(range(0, len(prompts), batch_size)):
            ...
            if prompts_B is not None:
                prompt_B = prompts_B[batch_start : batch_start + batch_size]
            else:
                prompt_B = None
