...
if __name__ == "__main__":
    SAVE_DIR = (
        f"results_if/{BENCHMARK}/neg_prompt/"
        f"{START_TIME}_"
        f"seed_{SEED}_"
    ...
    with accelerator.split_between_processes(prompts_indices) as device_indices:
        ...
        n = torch.stack([noises[i] for i in device_indices])
        n_u = torch.stack([noises_upsample[i] for i in device_indices])
        p_neg = [prompts_neg[i] for i in device_indices]
        neg_images = sample(
            pipe,
            pipe_upsample,
            [p["prompt"] for p in p_A],
        ...
        all_neg_images.extend(neg_images)
    accelerator.wait_for_everyone()
    ...
    if accelerator.is_main_process:
        np.save(os.path.join(SAVE_DIR, "Neg.npy"), all_neg_images)

    if accelerator.is_main_process:
        neg_images_stats = calculate_metrics(
            all_original_images_A,
            original_images_A_feats,
            all_neg_images,
            [p["text"] for p in prompts_A],
        ...
        neg_images_df["Block_patched"] = [f"OURS" for _ in range(len(prompts_A))]
