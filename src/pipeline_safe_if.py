...
class SafeIFPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def prepare_intermediate_images(
        ...
        if latents is None:
            ...
            intermediate_images = latents.to(device)
    ...
    def clean_cache(self):
        for name, module in self.unet.named_modules():
            if name.endswith("attn2"):
                module.cache_k = None
                module.cache_v = None
                module.curr_cache_k = None
                module.curr_cache_v = None
        print("Cache cleaned.")
    ...
    def __call__(
        ...
        safety_concept_embeds = None
        if enable_safety_guidance:
            safety_concept_embeds, negative_safety_concept_embeds = self.encode_prompt(
                self.safety_concept,
                do_classifier_free_guidance,
                device=device,
                num_images_per_prompt=len(prompt_embeds),
                clean_caption=clean_caption,
            )
            # print(f"{safety_concept_embeds.shape=}")
            # print(f"{negative_safety_concept_embeds.shape=}")
            ...
            if enable_safety_guidance:
                prompt_embeds = torch.cat(
                    [
                        negative_prompt_embeds,
                        prompt_embeds,
                        # negative_safety_concept_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                model_input = (
                    torch.cat(
                        [intermediate_images] * (3 if enable_safety_guidance else 2)
                    )
                    if do_classifier_free_guidance
                ...
                if do_classifier_free_guidance:
                    if enable_safety_guidance:
                        ...
                        noise_pred_safety_concept, predicted_variance_safety_concept = (
                            noise_pred_safety_concept.split(model_input.shape[1], dim=1)
                        )
