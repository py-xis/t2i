...
def noise_regularization(e_t, noise_pred_optimal, lambda_kl, lambda_ac, num_reg_steps, num_ac_rolls, generator=None):
...
def inversion_step(
    ...
    extra_step_kwargs = {}
    avg_range = (
        pipe.cfg.average_first_step_range if t.item() < first_step_max_timestep else pipe.cfg.average_step_range
    )
    num_renoise_steps = (
        ...
        with torch.no_grad():
            ...
            if pipe.cfg.noise_regularization_num_reg_steps > 0 and i == 0:
                ...
                if added_cond_kwargs is not None:
                    ...
                    added_cond_kwargs_in["text_embeds"] = torch.cat(
                        [added_cond_kwargs["text_embeds"], added_cond_kwargs["text_embeds"]]
                    )
                    added_cond_kwargs_in["time_ids"] = torch.cat(
            ...
            if pipe.cfg.noise_regularization_num_reg_steps > 0 and i == 0:
                ...
                if pipe.do_classifier_free_guidance:
                    ...
                    noise_pred_optimal = noise_pred_optimal_uncond + pipe.guidance_scale * (
                        noise_pred_optimal_text - noise_pred_optimal_uncond
                    )
