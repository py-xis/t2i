...
prompt_A_template = "a photo of a bear holding a board saying '<text>'"
prompt_B_template = "<text>"
DIR_PATH = f"./results/sdxl_layers_last_A_template_{''.join(c for c in prompt_A_template.replace(' ', '_') if c not in {'<', '>'})}_B_template_{''.join(c for c in prompt_B_template.replace(' ', '_') if c not in {'<', '>'})}_without_ciapki"
...
for i, text_A in enumerate(TEXTS):
    for j, text_B in enumerate(TEXTS):
        A_prompts.append(
            {
                "text": text_A,
                "prompt": prompt_A_template.replace("<text>", text_A),
            }
        )
        B_prompts.append(
            {
                "text": text_B,
                "prompt": prompt_B_template.replace("<text>", text_B),
            }
        )
print(A_prompts)
print(B_prompts)
...
for idx, b_layers in enumerate(CAUSAL_LAST_TRACING):
    ...
    grid = image_grid(images, len(TEXTS), len(TEXTS))
