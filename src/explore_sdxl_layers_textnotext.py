...
DEVICE = "cuda"
OBJECTS = ["bear"]
TEXTS = ["hello world", "diffusion", "text to image", "ICLR", "deep learning", "love"]

base_template = "a photo of a <object> with a text '<text>'"
target_template = "a photo of a <object>"
DIR_PATH = f"./results/sdxl_base_{base_template.replace(' ', '_').replace('<', '').replace('>', '')}_target_{target_template.replace(' ', '_').replace('<', '').replace('>', '')}"
...
set_seed(SEED)
# OBJECTS = ["bear", "cat", "dog", "man", "woman", "child", "robot"]
base_prompts = []
target_prompts = []
...
for obj in OBJECTS:
    for text in TEXTS:
        base_prompts.append(
            {
                "text": text,
                "prompt": base_template.replace("<object>", obj).replace(
                    "<text>", text
                ),
            }
        )
        target_prompts.append(
            {"text": "", "prompt": target_template.replace("<object>", obj)}
        )
device = DEVICE
...
noise = noise.repeat(len(target_prompts) // N_SAMPLES_PER_PROMPT, 1, 1, 1)
print(noise.shape)
...
for idx, b_layers in enumerate(CAUSAL_BLOCKS_TRACING):
    ...
    # for b_layers in CAUSAL_LAYERS_TRACING:
    ...
    images = batch_sample(
        pipeline=pipe,
        noise=noise,
        prompts_A=[prompt["prompt"] for prompt in base_prompts],
        generator=generator,
        num_inference_steps=N_STEPS,
        prompts_B=[prompt["prompt"] for prompt in target_prompts],
        batch_size=BATCH_SIZE,
        B_layers=b_layers,
    )

    texts_A = [prompt["text"] for prompt in base_prompts]
    texts_B = [prompt["text"] for prompt in target_prompts]
    prompts_A = [prompt["prompt"] for prompt in base_prompts]
    prompts_B = [prompt["prompt"] for prompt in target_prompts]
    ...
    for i in range(0, len(images), len(TEXTS)):
        grid = image_grid(images[i : i + len(TEXTS)], 1, len(TEXTS))
        grid.save(f"{LAYERS_PATH}/{OBJECTS[i//len(TEXTS)]}.png")
