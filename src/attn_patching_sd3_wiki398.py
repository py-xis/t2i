...
SAVE_DIR = f"results_sd3/wiki398_T_{START_TIME}"
ATTENTIONS_TO_PATCH = [
    10,
...
def prepare_name(attention_indices, head_indices):
    name = "T"
    for idx in attention_indices:
        name += str(idx)
    if head_indices is not None:
        head_str = "".join(str(idx) for idx in head_indices)
        name += f"H{head_str}"
    return name
...
patched_images_df["Block_patched"] = [
    prepare_name(ATTENTIONS_TO_PATCH, HEADS_TO_PATCH) for _ in range(len(prompts_A))
]
...
np.save(
    os.path.join(
        SAVE_DIR,
        f"{prepare_name(ATTENTIONS_TO_PATCH, HEADS_TO_PATCH)}.npy",
    ),
