...
prompts_A, prompts_B = prepare_prompts_wiki1000("data/wiki1000.txt")
...
logging.info("Sampling patched images ...")
...
original_images_A_df = pd.DataFrame(
    ...
    index=["Original_images_A" for _ in range(len(prompts_A))],
)
original_images_B_df = pd.DataFrame(
    original_images_B_metrics,
    index=["Original_images_B" for _ in range(len(prompts_A))],
)
patched_images_df = pd.DataFrame(
    patched_images_metrics, index=["Patched_block_10" for _ in range(len(prompts_A))]
)

# Concatenate all DataFrames
all_metrics_df = pd.concat(
    [original_images_A_df, original_images_B_df, patched_images_df]
)
...
np.save(os.path.join(SAVE_DIR, "original_images_A.npy"), original_images_A)
np.save(os.path.join(SAVE_DIR, "original_images_B.npy"), original_images_B)
np.save(os.path.join(SAVE_DIR, "patched_images_10.npy"), patched_images)
